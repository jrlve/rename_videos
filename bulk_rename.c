...
void renameVideos(list *paths, list *trials, list *extensions) {
  for (int i = 0; i < paths->count; i++) {
    ...
    char *file_base = allocateString(*(paths->names + i));
    //char *folder = concat(timestamp, prefix);
    char *fully_qualified_folder = concat(concat(timestamp, prefix), file_base);
    makedir(fully_qualified_folder);

    printf("%s\n", fully_qualified_folder);
    for (int j = 0; j < min(heap->size, trials->count); j++) {
      char *base = concat(allocateString("/"), allocateString(fully_qualified_folder));
      char *new_name = concat(allocateString(*((trials->names) + j)), base);
      //printf("%s\n", base);
      char *old_name = concat(allocateString((*((heap->elements) + j)).value), concat(allocateString("/"), allocateString(*(paths->names + i))));
      printf("old name: %s\n", old_name);
      new_name = concat(extractExtension(old_name), concat(allocateString("."), new_name));
      copyFile(old_name, new_name);
      printf("old name: %s, new_name: %s \n", old_name, new_name);
    }
  }
}
